...
public class MvcController3{
   ...
   *  2. 형식
   *     @RequestParam(value = "파라미터이름", required = 파라미터필수여부, defaultValue = "기본값")
   *  3. 요청 파라미터가 정상적으로 전달된다면 @RequestParam 을 생략할 수도 있다.
   *     ex) public String param2(int page, String sort){}
   * 
   */
  
  @RequestMapping("/param2")
  public String param2(@RequestParam(value = "page", required= false, defaultValue = "1") int page // 요청 파라미터 page (디폴트1)
      , @RequestParam(value = "sort", required= false, defaultValue= "DESC") String sort) {        // 요청 파라미터 sort (디폴트 DESC)
  //  page 파라미터는 int page 에 저장 , sort 파라미터는 String sort 에 저장함. required=? (이 파라미터가 필수니? true & false)
